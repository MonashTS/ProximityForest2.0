...
namespace pyPF {
    ...
    inline void submod_tseries(py::module& m){
        ...
        py::class_<pyTSeries>(m, "TSeries", py::buffer_protocol())
                ...
                .def("__getitem__", [](const pyTSeries &s, py::ssize_t i) {
                    if(s.nb_dimensions()>1){ throw std::invalid_argument("A multivariate series must be accessed by [track, idx]"); }
                    if (i<0 || i>=(ssize_t)s.length()){ throw py::index_error(); }
                    return s.data()[i];
                })
    ...
    } // End of submod_tseries(py::module& m)

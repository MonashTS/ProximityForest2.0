...
public class ADTW {
    public static double distance(double[] lines, double[] cols, double weight, double cutoff) {
        ...
        double ub = cutoff + EPSILON - Tools.cost(lines[nblines - 1], cols[nbcols - 1]);
        ...
        // Initialisation of the first line.
        {
            ...
            for (j = 1; j == curr_pp && j < nbcols; ++j) {
                cost = cost + Tools.cost(l0, cols[j]) + weight; // Left: penalty
                buffers[c + j] = cost;
                if (cost <= ub) {
                    ++curr_pp;
                }
        ...
        for (; i < nblines; ++i) {
            ...
            {
                cost = buffers[p + j] + Tools.cost(li, cols[j]) + weight; // Top: penalty
                buffers[c + j] = cost;
                if (cost <= ub) {
                    curr_pp = j + 1;
                } else {
            ...
            for (; j == next_start && j < prev_pp; ++j) {
                final double d = Tools.cost(li, cols[j]);
                cost = min(
            ...
            for (; j == curr_pp && j < nbcols; ++j) {
                cost = cost + Tools.cost(li, cols[j]) + weight;// Left: penalty
                buffers[c + j] = cost;
                if (cost <= ub) {
                    ++curr_pp;
                }

...
public class CDTW {
    ...
    public static double distance(double[] lines, double[] cols, int w, double cutoff) {
        ...
        double ub = cutoff + Tools.EPSILON - Tools.cost(lines[nblines - 1], cols[nbcols - 1]);
        ...
        for (; i < nblines; ++i) {
            ...
            for (; j == next_start && j < prev_pp; ++j) {
                double d = Tools.cost(li, cols[j]);
                cost = Double.min(buffers[p + j - 1], buffers[p + j]) + d;
                buffers[c + j] = cost;
                if (cost <= ub) {
                    curr_pp = j + 1;
                } else {
            ...
            if (j < jStop) { // If so, two cases.
                ...
                if (j == next_start) { // Case 1: Advancing next start: only diag.
                    ...
                    cost = Double.min(cost, buffers[p + j - 1]) + d;
                    buffers[c + j] = cost;
                    if (cost <= ub) {
                        curr_pp = j + 1;
                    }

...
public class CDTW_ge extends CDTW {

    public static double distance(double[] lines, double[] cols, int w, double cutoff, double ge) {
        ...
        double ub = cutoff + Tools.EPSILON - Tools.cost(lines[nblines - 1], cols[nbcols - 1], ge);
        ...
        for (; i < nblines; ++i) {
            ...
            for (; j == next_start && j < prev_pp; ++j) {
                double d = Tools.cost(li, cols[j], ge);
            ...
            for (; j < prev_pp; ++j) {
                double d = Tools.cost(li, cols[j], ge);
                cost = Tools.min(cost, buffers[p + j - 1], buffers[p + j]) + d;
                buffers[c + j] = cost;
                if (cost <= ub) {
                    curr_pp = j + 1;
                }

...
public class LCSS {
    ...
    public static double distance(double[] lines, double[] cols, double epsilon, int w, double cutoff) {
        ...
        cutoff = Double.max(0, Double.min(cutoff, 1));
        ...
        for (; i < nblines; ++i) {
            ...
            for (j = jStart; j < jStop; ++j) {
                if (sim(li, cols[j], epsilon)) {
                    ...
                    buffers[c + j] = Tools.max(buffers[c + j - 1], buffers[p + j - 1], buffers[p + j]);
                }
            }
        }

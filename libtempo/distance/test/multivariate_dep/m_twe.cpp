...
constexpr double QNAN = libtempo::utils::PINF<double>;
    ...
    for (const auto &s : fset) {
      for (const double nu : nus) {
        for (const double la : lambdas) {
          ...
          REQUIRE(twe_ref_v == 0);
          const auto twe_v = twe<double>(l, l, nu, twe_warp_ad2(s, ndim, nu, la), twe_warp_ad2(s, ndim, nu, la), ad2N<double>(s, s, ndim), INF);
          REQUIRE(twe_v == 0);
        }
      }
    }
      ...
      for (const double la : lambdas) {
        for (size_t i = 0; i < nbitems - 1; ++i) {
          ...
          // Check Uni
          {
            ...
            const double twe_tempo_v = twe(l1, l1, nu, twe_warp_ad2(s1, 1, nu, la), twe_warp_ad2(s2, 1, nu, la), ad2N<double>(s1, s2, 1), INF);
          ...
          // Check Multi
          {
            ...
            INFO("l = " << l << " ndim = " << ndim)
            const double twe_tempo_v = twe<double>(l, l, nu, twe_warp_ad2(s1, ndim, nu, la), twe_warp_ad2(s2, ndim, nu, la), ad2N<double>(s1, s2, ndim), INF);
            REQUIRE(twe_ref_v == twe_tempo_v);
          }
        }
      }
      ...
      for (auto la : lambdas) {
        ...
        for (size_t i = 0; i < nbitems; i += 3) {
          ...
          for (size_t j = 0; j < nbitems; j += 5) {
            ...
            // --- --- --- --- --- --- --- --- --- --- --- ---
            const auto
              v = twe<double>(l, l, nu, twe_warp_ad2(s1, ndim, nu, la), twe_warp_ad2(s2, ndim, nu, la), ad2N<double>(s1, s2, ndim), INF);
            if (v < bsf) {
              idx = j;
              bsf = v;
            }
            REQUIRE(idx_ref == idx);
            // --- --- --- --- --- --- --- --- --- --- --- ---
            const auto v_tempo = twe<double>(l, l, nu, twe_warp_ad2(s1, ndim, nu, la), twe_warp_ad2(s2, ndim, nu, la), ad2N<double>(s1, s2, ndim), bsf_tempo);
    ...
    for (const auto& s: fset) {
      for (auto nu: nus) {
        for (auto la: lambdas) {
          const double twe_ref_v = twe_matrix(s, s, ndim, nu, la);
          REQUIRE(twe_ref_v==0);
          const auto twe_v = twe<double>(ld(s), ld(s), nu, twe_warp_ad2(s, ndim, nu, la), twe_warp_ad2(s, ndim, nu, la), ad2N<double>(s, s, ndim), INF);
          REQUIRE(twe_v==0);
        }
      }
    }
      ...
      for (auto nu: nus) {
        for (auto la: lambdas) {
          // Check Uni
          {
            const double twe_ref_v = twe_matrix(s1, s2, 1, nu, la);
            const double twe_ref_uni_v = twe_matrix_uni(s1, s2, nu, la);
            const auto twe_tempo_v = twe<double>(s1.size(), s2.size(), nu, twe_warp_ad2(s1, 1, nu, la), twe_warp_ad2(s2, 1, nu, la), ad2N<double>(s1, s2, 1), INF);
            REQUIRE(twe_ref_v==twe_ref_uni_v);
            REQUIRE(twe_ref_v==twe_tempo_v);
          }
          // Check Multi
          {
            const double twe_ref_v = twe_matrix(s1, s2, ndim, nu, la);
            INFO("Exact same operation order. Expect exact floating point equality.")
            const auto twe_tempo_v = twe<double>(ld(s1), ld(s2), nu, twe_warp_ad2(s1, ndim, nu, la), twe_warp_ad2(s2, ndim, nu, la), ad2N<double>(s1, s2, ndim), INF);
            REQUIRE(twe_ref_v==twe_tempo_v);
          }
        }
      }
      ...
      for (size_t j = 0; j<nbitems; j += 5) {
        ...
        for (auto nu: nus) {
          for (auto la: lambdas) {
            ...
            // --- --- --- --- --- --- --- --- --- --- --- ---
            const auto v = twe<double>(ld(s1), ld(s2), nu, twe_warp_ad2(s1, ndim, nu, la), twe_warp_ad2(s2, ndim, nu, la), ad2N<double>(s1, s2, ndim), INF);
            if (v<bsf) {
              idx = j;
              bsf = v;
            }
            REQUIRE(idx_ref==idx);
            // --- --- --- --- --- --- --- --- --- --- --- ---
            const auto v_tempo = twe<double>(ld(s1), ld(s2), nu, twe_warp_ad2(s1, ndim, nu, la), twe_warp_ad2(s2, ndim, nu, la), ad2N<double>(s1, s2, ndim), bsf_tempo);
            if (v_tempo<bsf_tempo) {
              idx_tempo = j;
              bsf_tempo = v_tempo;
            }

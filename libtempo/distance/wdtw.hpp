  ...
  [[nodiscard]] F wdtw(
    ...
    if (nblines == 0 && nbcols == 0) { return 0; }
      ...
      if (std::isinf(ub)) {
        ...
        if (nblines < nbcols) {
          for (size_t j{nblines}; j < nbcols; ++j) { ub = ub + dist(nblines - 1, j); }
        }
          // Fewer columns than lines: complete the last column (advance in the lines)
        else if (nbcols < nblines) {
          for (size_t i{nbcols}; i < nblines; ++i) { ub = ub + dist(i, nbcols - 1); }
        }
      } else if (std::isnan(ub)) { ub = INF; }
      // ub computed
      return internal::wdtw<F>(nblines, nbcols, dist, ub, buffer_v);
    }
  }

...
namespace libtempo::distance {
  ...
  namespace internal {
    ...
    [[nodiscard]] inline FloatType norm(size_t length1, size_t length2, FDist dist, const FloatType cutoff) {
      // --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
      ...
      const FloatType lastA = dist(length1-1, length1-1);
     ...
     */
    template<typename FloatType, typename FDist>
    [[nodiscard]] inline FloatType norm(size_t length1, size_t length2, FDist dist) {
      // --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
  ...
  */
  template<typename FloatType, typename FDist>
  [[nodiscard]] FloatType norm(size_t length1, size_t length2, FDist dist, FloatType ub) {
    if (std::isinf(ub) || std::isnan(ub)) { return internal::norm<FloatType, FDist>(length1, length2, dist); }
    else { return internal::norm<FloatType, FDist>(length1, length2, dist, ub); }
  }

  /// Helper with a distance builder 'mkdist'
  template<typename FloatType>
  [[nodiscard]] inline FloatType norm(const auto& s1, const auto& s2, auto mkdist, FloatType ub) {
    return norm(s1.size(), s2.size(), mkdist(s1, s2), ub);
  }

  /// Helper with the sqed as the default distance builder, and a default ub at +INF
  template<typename FloatType>
  [[nodiscard]] inline FloatType norm(const auto& s1, const auto& s2, FloatType ub = utils::PINF<FloatType>) {
    return norm(s1.size(), s2.size(), distance::sqed<FloatType>(s1, s2), ub);
  }

  /// Multidimensional helper, with a distance builder 'mkdist'
  template<typename FloatType>
  [[nodiscard]] inline FloatType norm(const auto& s1, const auto& s2, size_t ndim, auto mkdist, FloatType ub) {
    return norm(s1.size()/ndim, s2.size()/ndim, mkdist(s1, s2, ndim), ub);
  }

  /// Multidimensional helper, with a distance builder 'mkdist'
  template<typename FloatType>
  [[nodiscard]] inline FloatType norm(const auto& s1, const auto& s2, size_t ndim, FloatType ub = utils::PINF<FloatType>) {
    return norm(s1.size()/ndim, s2.size()/ndim, distance::sqed<FloatType>(s1, s2, ndim), ub);
  }

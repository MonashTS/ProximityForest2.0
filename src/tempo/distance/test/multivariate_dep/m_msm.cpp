  ...
  SECTION("MSM(s,s) == 0") {
    for (const auto &s : fset) {
      for (auto c : msm_costs) {
        ...
        const auto msm_v = msm<double>(l, l, msm_lines_ed(s, s, ndim, c), msm_cols_ed(s, s, ndim, c), msm_diag_ed<double>(s, s, ndim), INF);
        REQUIRE(msm_v == 0);
      }
    }
  }
    ...
    for (size_t i = 0; i < nbitems - 1; ++i) {
      ...
      for (auto c : msm_costs) {
        ...
        // Note that we re-coded the univariate with the Euclidean distance instead of using the L1 norm
        {
          ...
          const double
            msm_tempo_v = msm<double>(l1, l1, msm_lines_ed(s1, s2, 1, c), msm_cols_ed(s1, s2, 1, c), msm_diag_ed<double>(s1, s2, 1), INF);
        ...
        // Ok, test the multivariate
        {
          ...
          const double msm_tempo_v =
            msm<double>(l, l, msm_lines_ed(s1, s2, ndim, c), msm_cols_ed(s1, s2, ndim, c), msm_diag_ed<double>(s1, s2, ndim), INF);
          REQUIRE(msm_ref_v == msm_tempo_v);
        }
      }
    }
    ...
    for (auto c : msm_costs) {
      ...
      for (size_t i = 0; i < nbitems; i += 3) {
        ...
        for (size_t j = 0; j < nbitems; j += 5) {
          ...
          // --- --- --- --- --- --- --- --- --- --- --- ---
          const auto v_tempo =
            msm<double>(l, l, msm_lines_ed(s1, s2, ndim, c), msm_cols_ed(s1, s2, ndim, c), msm_diag_ed<double>(s1, s2, ndim), bsf_tempo);
  ...
  SECTION("MSM(s,s) == 0") {
    for (const auto &s : fset) {
      for (auto c : msm_costs) {
        ...
        const auto
          msm_v = msm<double>(ld(s), ld(s), msm_lines_ed(s, s, ndim, c), msm_cols_ed(s, s, ndim, c), msm_diag_ed<double>(s, s, ndim), INF);
        REQUIRE(msm_v == 0);
      }
    }
  }
    ...
    for (size_t i = 0; i < nbitems - 1; ++i) {
      ...
      for (auto c : msm_costs) {
        ...
        // Note that we recoded the univariate with the Euclidean distance instead of using the L1 norm
        {
          ...
          REQUIRE(msm_ref_v == msm_ref_univ);
          const double msm_tempo_v =
            msm<double>(s1.size(), s2.size(), msm_lines_ed(s1, s2, 1, c), msm_cols_ed(s1, s2, 1, c), msm_diag_ed<double>(s1, s2, 1), INF);
          REQUIRE(msm_ref_v == msm_tempo_v);
        }
        ...
        {
          const double msm_ref_v = msm_matrix(s1, s2, ndim, c);
          const double msm_tempo_v = msm<double>(ld(s1),
                                                 ld(s2),
                                                 msm_lines_ed(s1, s2, ndim, c),
                                                 msm_cols_ed(s1, s2, ndim, c),
                                                 msm_diag_ed<double>(s1, s2, ndim),
                                                 INF);
          REQUIRE(msm_ref_v == msm_tempo_v);
        }
      }
    ...
    for (auto c : msm_costs) {
      ...
      for (size_t i = 0; i < nbitems; i += 3) {
        ...
        for (size_t j = 0; j < nbitems; j += 5) {
          ...
          // --- --- --- --- --- --- --- --- --- --- --- ---
          const auto v_tempo = msm<double>(ld(s1),
                                           ld(s2),
                                           msm_lines_ed(s1, s2, ndim, c),
                                           msm_cols_ed(s1, s2, ndim, c),
                                           msm_diag_ed<double>(s1, s2, ndim),

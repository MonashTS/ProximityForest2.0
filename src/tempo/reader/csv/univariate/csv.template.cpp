...
namespace tempo::reader::univariate {
  ...
  tempo::reader::Result<F> read_csv(
    ...
    CSVReaderParam const& params
  ) {

    using SData = typename tempo::reader::ReaderData<F>::SData;
    ...
    if (!params.comment_skip.empty()) {
      ...
      lreader_params.mCommentPrefix = params.comment_skip;
    } else {

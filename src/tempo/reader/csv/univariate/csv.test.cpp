...
TEST_CASE("CSV reader") {
  ...
  tempo::reader::Result<F> vresult = read_csv<F>(input, tempo::LabelEncoder(), params);
  tempo::reader::ReaderData<F> const& result = std::get<1>(vresult);

  // Check that we read the correct number lines
  REQUIRE(result.series.size()==5);

  // Check the label encoder
  tempo::LabelEncoder const& e = result.encoder;

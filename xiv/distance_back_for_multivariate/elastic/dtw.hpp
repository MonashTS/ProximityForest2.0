...
namespace tempo::distance {
  ...
  namespace WR {
    ...
    namespace internal {
      /** Same as above, return a WarpingResult; contains the smallest window for which the result is still valid.
       * Uses an extra buffer to perform the window validity computation */
      [[nodiscard]] inline WarpingResult dtw(const size_t nblines,
                                             const size_t nbcols,
                                             CFun auto dist,
                                             const size_t w,
                                             const F cutoff,
        ...
        for (; i<nblines; ++i) {
          ...
          if (j<jStop) { // If so, two cases.
            ...
            if (j==next_start) {
              // But only if we are above the original UB
              // Else set the next starting point to the last valid column
              if (cost>cutoff) { return WarpingResult(); } else { next_start = nbcols - 1; }
            }
          }
          // --- --- --- Stage 4: After the previous pruning point: only prev.
          // Go on while we advance the curr_pp; if it did not advance, the rest of the line is guaranteed to be > ub.
        ...
        // Finalisation
        // Check for last alignment (i==nblines implied, Stage 4 implies j<=nbcols). Cost must be <= original bound.
        if (j==nbcols&&cost<=cutoff) { return WarpingResult(cost, mw); } else { return WarpingResult(); }
      }

    } // End of namespace internal

    [[nodiscard]] WarpingResult dtw(size_t nblines,
                                    size_t nbcols,
                                    CFun auto dist,
                                    size_t w,
                                    F ub,
      ...
      using utils::PINF;
      if (nblines==0&&nbcols==0) { return WarpingResult(0,0); }
      else if ((nblines==0)!=(nbcols==0)) { return WarpingResult(); }
      else {
        // Check that the window allows for an alignment
        // If this is accepted, we do not need to check the window when computing a new UB
        const auto m = std::min(nblines, nbcols);
        const auto M = std::max(nblines, nbcols);
        if (M - m>w) { return WarpingResult(); }
        // Compute a cutoff point using the diagonal
        if (std::isinf(ub)) {
          ub = 0;
          // Cover diagonal
        ...
        // ub computed
        return internal::dtw(nblines, nbcols, dist, w, ub, buffer_v, buffer_wv);
      }
    }

    /// Helper without having to provide buffers
    [[nodiscard]] inline WarpingResult dtw(size_t nblines, size_t nbcols, CFun auto dist, size_t w, F ub) {
      std::vector<F> v;
      std::vector<size_t> wv;
      return dtw(nblines, nbcols, dist, w, ub, v, wv);
    }

    /// Helper for TSLike, without having to provide buffers
    template<TSLike T>
    [[nodiscard]] inline WarpingResult
    dtw(const T& lines, const T& cols, CFunBuilder<T> auto mkdist, size_t w, F ub) {
      const auto ls = lines.length();
      const auto cs = cols.length();
      const CFun auto dist = mkdist(lines, cols);
      std::vector<F> v;
      std::vector<size_t> wv;
      return dtw(ls, cs, dist, w, ub, v, wv);
    }

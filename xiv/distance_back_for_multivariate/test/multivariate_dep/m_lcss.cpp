...
namespace ref {
  ...
  double lcss_matrix_uni(const vector<double>& series1, const vector<double>& series2, double epsilon, size_t w_) {
    ...
    if (matrix[length1][length2]==-1) { return tempo::utils::PINF; }
    ...
    for (const auto& s : fset) {
      for (double e : epsilons) {
        for (double wr : wratios) {
          ...
          const double lcss_ref_v = ref::lcss_matrix(s, s, ndim, e, w);
          REQUIRE(lcss_ref_v==0);

          const auto lcss_v = lcss(l, l, simN(s, s, ndim, e), w, PINF);
          REQUIRE(lcss_v==0);
        }
      }
    }
      ...
      for (size_t i = 0; i<nbitems - 1; ++i) {
        ...
        for (double e : epsilons) {
          for (double wr : wratios) {
            ...
            // Check Uni
            {
              const double lcss_ref_v = ref::lcss_matrix(s1, s2, 1, e, w);
              const double lcss_ref_uni_v = ref::lcss_matrix_uni(s1, s2, e, w);
              const auto lcss_tempo_v = lcss(l1, l1, simN(s1, s2, 1, e), w, PINF);
            ...
            // Check Multi
            {
              const double lcss_ref_v = ref::lcss_matrix(s1, s2, ndim, e, w);
              INFO("Exact same operation order. Expect exact floating point equality.")
              const auto lcss_tempo_v = lcss(l, l, simN(s1, s2, ndim, e), w, PINF);
              REQUIRE(lcss_ref_v==lcss_tempo_v);
            }
          }
        }
        ...
        for (size_t j = 0; j<nbitems; j += 5) {
          ...
          for (double e : epsilons) {
            for (double wr : wratios) {
              const auto w = (size_t)(wr*mocker._fixl);
              // --- --- --- --- --- --- --- --- --- --- --- ---
              const double v_ref = ref::lcss_matrix(s1, s2, ndim, e, w);
              if (v_ref<bsf_ref) {
                idx_ref = j;
                bsf_ref = v_ref;
              }
              // --- --- --- --- --- --- --- --- --- --- --- ---
              const auto v = lcss(l, l, simN(s1, s2, ndim, e), w, PINF);
              if (v<bsf) {
                idx = j;
                bsf = v;
              }
              REQUIRE(idx_ref==idx);
              // --- --- --- --- --- --- --- --- --- --- --- ---
              const auto v_tempo = lcss(l, l, simN(s1, s2, ndim, e), w, bsf_tempo);
    ...
    for (const auto& s : fset) {
      for (double e : epsilons) {
        for (double wr : wratios) {
          ...
          const double lcss_ref_v = ref::lcss_matrix(s, s, ndim, e, w);
          REQUIRE(lcss_ref_v==0);
          const auto lcss_v = lcss(ld(s), ld(s), simN(s, s, ndim, e), w, PINF);
          REQUIRE(lcss_v==0);
        }
      }
    }
      ...
      for (double e : epsilons) {
        for (double wr : wratios) {
          ...
          // Check Uni
          {
            const double lcss_ref_v = ref::lcss_matrix(s1, s2, 1, e, w);
            const double lcss_ref_uni_v = ref::lcss_matrix_uni(s1, s2, e, w);
            const auto lcss_tempo_v = lcss(s1.size(), s2.size(), simN(s1, s2, 1, e), w, PINF);
            REQUIRE(lcss_ref_v==lcss_ref_uni_v);
            REQUIRE(lcss_ref_v==lcss_tempo_v);
          }
          ...
          // Check Multi
          {
            ...
            const auto lcss_tempo_v = lcss(ld(s1), ld(s2), simN(s1, s2, ndim, e), w, PINF);
            REQUIRE(lcss_ref_v==lcss_tempo_v);
          }
        }
      }
      ...
      for (double wr : wratios) {
        ...
        for (size_t i = 99; i<nbitems; i += 3) {
          ...
          for (size_t j = 10; j<nbitems; j += 5) {
            ...
            // --- --- --- --- --- --- --- --- --- --- --- ---
            const auto v = lcss(ld(s1), ld(s2), simN(s1, s2, ndim, e), w, PINF);
            if (v<bsf) {
              idx = j;
              bsf = v;
            }
            ...
            // --- --- --- --- --- --- --- --- --- --- --- ---
            const auto v_tempo = lcss(ld(s1), ld(s2), simN(s1, s2, ndim, e), w, bsf_tempo);
            if (v_tempo<bsf_tempo) {
              idx_tempo = j;
              bsf_tempo = v_tempo;
            }

...
namespace ref {
  ...
  inline double get_cost(double xnew, double xi, double yj, double cost) {
    ...
    // For comparison with euclidean distance
  ...
  double msm_matrix(const vector<double>& series1, const vector<double>& series2, size_t dim, double c) {
    ...
    const long maxLength = max(length1, length2);
    vector<std::vector<double>> cost_matrix(maxLength, std::vector<double>(maxLength, PINF));
...
TEST_CASE("Multivariate Dependent MSM Fixed length", "[msm][multivariate]") {
  ...
  Mocker mocker;
  mocker._dim = ndim;
  const size_t l = mocker._fixl;
  const size_t l1 = l*mocker._dim;
  ...
  SECTION("MSM(s,s) == 0") {
    for (const auto& s : fset) {
      for (auto c : msm_costs) {
        const double msm_ref_v = ref::msm_matrix(s, s, ndim, c);
        REQUIRE(msm_ref_v==0);

        const auto
          msm_v = msm(l, l, msm_lines_ed(s, s, ndim, c), msm_cols_ed(s, s, ndim, c), msm_diag_ed(s, s, ndim), PINF);
        REQUIRE(msm_v==0);
      }
    }
  }
  ...
  SECTION("MSM(s1, s2)") {
    for (size_t i = 0; i<nbitems - 1; ++i) {
      ...
      for (auto c : msm_costs) {
        ...
        // Note that we re-coded the univariate with the Euclidean distance instead of using the L1 norm
        {
          const double msm_ref_v = ref::msm_matrix(s1, s2, 1, c);
          const double msm_ref_univ = ref::msm_matrix_uni(s1, s2, c);
          REQUIRE(msm_ref_v==msm_ref_univ);
          const double msm_tempo_v =
            msm(l1, l1, msm_lines_ed(s1, s2, 1, c), msm_cols_ed(s1, s2, 1, c), msm_diag_ed(s1, s2, 1), PINF);
          REQUIRE(msm_ref_v==msm_tempo_v);
        }

        // Ok, test the multivariate
        {
          const double msm_ref_v = ref::msm_matrix(s1, s2, ndim, c);
          const double msm_tempo_v =
            msm(l, l, msm_lines_ed(s1, s2, ndim, c), msm_cols_ed(s1, s2, ndim, c), msm_diag_ed(s1, s2, ndim), PINF);
          REQUIRE(msm_ref_v==msm_tempo_v);
        }
      }
    }
    ...
    for (auto c : msm_costs) {
      ...
      for (size_t i = 0; i<nbitems; i += 3) {
        ...
        for (size_t j = 0; j<nbitems; j += 5) {
          ...
          // --- --- --- --- --- --- --- --- --- --- --- ---
          const double v_ref = ref::msm_matrix(s1, s2, ndim, c);
          if (v_ref<bsf_ref) {
            idx_ref = j;
            bsf_ref = v_ref;
          }
          ...
          // --- --- --- --- --- --- --- --- --- --- --- ---
          const auto
            v = msm(l, l, msm_lines_ed(s1, s2, ndim, c), msm_cols_ed(s1, s2, ndim, c), msm_diag_ed(s1, s2, ndim), PINF);
          if (v<bsf) {
            idx = j;
            bsf = v;
          }
          ...
          // --- --- --- --- --- --- --- --- --- --- --- ---
          const auto v_tempo =
            msm(l, l, msm_lines_ed(s1, s2, ndim, c), msm_cols_ed(s1, s2, ndim, c), msm_diag_ed(s1, s2, ndim), bsf_tempo
            );
  ...
  SECTION("MSM(s,s) == 0") {
    for (const auto& s : fset) {
      for (auto c : msm_costs) {
        ...
        const auto msm_v =
          msm(ld(s), ld(s), msm_lines_ed(s, s, ndim, c), msm_cols_ed(s, s, ndim, c), msm_diag_ed(s, s, ndim), PINF);
        REQUIRE(msm_v==0);
      }
    }
  }
    ...
    for (size_t i = 0; i<nbitems - 1; ++i) {
      ...
      for (auto c : msm_costs) {
        ...
        // Note that we recoded the univariate with the Euclidean distance instead of using the L1 norm
        {
          ...
          REQUIRE(msm_ref_v==msm_ref_univ);
          const double msm_tempo_v =
            msm(s1.size(), s2.size(), msm_lines_ed(s1, s2, 1, c), msm_cols_ed(s1, s2, 1, c), msm_diag_ed(s1, s2, 1),
                PINF
            );
          REQUIRE(msm_ref_v==msm_tempo_v);
        }
        ...
        {
          const double msm_ref_v = ref::msm_matrix(s1, s2, ndim, c);
          const double msm_tempo_v = msm(ld(s1),
                                         ld(s2),
                                         msm_lines_ed(s1, s2, ndim, c),
                                         msm_cols_ed(s1, s2, ndim, c),
                                         msm_diag_ed(s1, s2, ndim),
                                         PINF
          );
          REQUIRE(msm_ref_v==msm_tempo_v);
        }
